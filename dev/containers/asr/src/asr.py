...
class Function:
    ...
    def cast(self, alias: str, msg: bytes) -> None:
        ...
        #array_bytes = base64.b64decode(response_data['audio'])        
        #logger.info("audio: {}".format(array_bytes) )
        audio_buffer = np.array(response_data["audio"], dtype=np.float32)
        logger.info("audio: {}".format(audio_buffer) )
        '''

        with open("recv_into_docer.wav", 'wb') as audio_file:
            audio_file.write(array_bytes)


        # understand how to elaborate the array_bytes inside whisper!

        dtype = np.int16  # 2 byte per campione (int16)
        audio_int = np.frombuffer(array_bytes, dtype=dtype)
        # Normalizza i dati in float32 (-1.0 a 1.0)
